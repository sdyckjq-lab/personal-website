...
export function GrowthSectionClient({ articles }: GrowthSectionClientProps) {
    ...
    return (
        <section id="growth" className="py-24 md:py-32 px-6 bg-card">
                ...
                <StaggerContainer className="grid md:grid-cols-2 gap-6">
                    {filteredArticles.map((article, index) => (
                        <StaggerItem key={index}>
                            ...
                            >
                                <Card
                                    className="neon-card p-6 bg-background border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 group cursor-pointer h-full"
                                >
